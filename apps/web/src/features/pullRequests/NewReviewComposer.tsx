/**
 * The review composer: the body of the review dock (spec v3 §4).
 *
 * It has two layouts over one submit machine. **Compact** is a single row —
 * selected chips, the preset, the prompt as a one-line summary, `Review` — so a
 * selection is reviewable without opening anything. **Full** is the editor:
 * multiline prompt, attachments, pre-flight notices, and the created-session
 * panel. Focusing the prompt expands; collapsing returns to the row, and is
 * offered only while nothing needs the room (no error, notice, attachment, or
 * freshly created session).
 *
 * The prompt is optional; pre-flight is what decides whether a session can be
 * created, and its notices are shown verbatim (see the refusal tests).
 */
...
import { groupSelectedByRepository, type SelectedPr } from "./lib/selection"
import { filesFromClipboard, useAttachments } from "./lib/useAttachments"
...
export interface NewReviewComposerProps {
  ...
  onRetryPresets?: () => void
  /** Fired once a session exists, so the dock can outlive the selection. */
  onCreated?: () => void
  /** Fired when the composer is put back to an empty state. */
  onReset?: () => void
}
...
const CARD_SHADOW =
  "shadow-[0_1px_2px_hsl(var(--foreground)/0.04),0_12px_32px_-16px_hsl(var(--foreground)/0.18)]"
...
export function NewReviewComposer({
  ...
  // Expanding is a deliberate act (focusing the prompt), so it takes focus with
  // it — otherwise the click that expanded would land on nothing.
  useEffect(() => {
    if (expanded) textareaRef.current?.focus()
  }, [expanded])

  const groups = useMemo(() => groupSelectedByRepository(selected), [selected])
      ...
      if (result.valid.length === 0) {
        ...
        setError(
          result.notices.length === 0
            ? "No pull request passed pre-flight. Check the selection and try again."
            : null,
        )
        setPhase("idle")
        return
  ...
  if (canCollapse && !expanded) {
    return (
      <div
        className={`animate-fade-up overflow-hidden rounded-2xl border border-border bg-card ${CARD_SHADOW}`}
      >
        <div className="flex flex-col gap-2 px-2.5 py-2.5 sm:flex-row sm:items-center sm:gap-2">
          <div className="flex min-w-0 items-center gap-1.5 overflow-x-auto sm:flex-1">
            {selected.map((item) => (
              <span
                key={item.url}
                className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-border bg-muted/50 py-1 pl-2.5 pr-1"
              >
                <a
                  href={item.url}
                  target="_blank"
          ...
          <div className="flex items-center gap-2 sm:min-w-0 sm:flex-1">
            <button
              ...
              className="min-w-0 flex-1 truncate rounded-lg px-2.5 py-1 text-left text-[13px] text-muted-foreground transition-colors hover:bg-muted/40"
            >
              {prompt.trim() || "Describe the review focus…"}
            </button>
  ...
  return (
    <div
      className={`overflow-hidden rounded-2xl border border-border bg-card ${CARD_SHADOW}`}
    >
      ...
      <div className="px-5 pb-3 pt-1">
        {selected.length === 0 ? (
          <p className="rounded-xl border border-dashed border-border bg-muted/20 px-3.5 py-3 text-2xs text-muted-foreground">
            Select pull requests from the list to review them.
          </p>
        ) : (
        ...
        <div className="flex min-w-0 items-center gap-1">
          ...
          {selected.length > 0 ? (
            <span className="hidden items-center gap-1.5 rounded-lg px-2 font-mono text-[11px] text-muted-foreground/80 sm:inline-flex">
              ...
              {selected.length} PR{selected.length === 1 ? "" : "s"} · {repoCount}{" "}
              repo{repoCount === 1 ? "" : "s"}
            </span>
          ) : null}
        </div>
