...
The startup hook builds the shared LLM client and database session factory; the
shutdown hook closes the HTTP pool and disposes the engine.
"""
...
async def on_startup(ctx: _StartupCtx) -> None:
    """Create the shared LLM client and database session factory."""
    ctx["review"] = ReviewContext(llm=build_llm_client(), config=get_worker_config())
