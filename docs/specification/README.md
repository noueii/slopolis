...
| 10.4 | Session creation form | [04-session-creation.md](./v1/features/04-session-creation.md) | yes — submit, auto-naming, per-target enqueue, the session's `main` agent run created at submit |
...
5. **GitHub reads stay per-repository and partially per-pull-request.** The picker's counts and
   CI state are cached for 30s, but a repository listing still reads each repository for its
   open-PR count and each pull request for diff size and checks: a large installation is slow on
   a cold cache and spends rate limit.
