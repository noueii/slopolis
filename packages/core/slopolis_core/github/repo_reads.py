...
class RepoReads:
    ...
    async def user_can_trigger(
        ...
        *,
        ...
        The bar follows the repository's visibility: public repos require
        **write**, private repos require **read**. Any auth failure fails closed
        (``False``).
        """
        level = "read" if private else "write"
        try:
